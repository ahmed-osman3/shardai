"""Ball detection via tiled YOLO inference.

Splits each frame into overlapping tiles, runs the model on each tile,
translates bounding boxes back to frame coordinates, and deduplicates
with NMS. Supports both a COCO-pretrained baseline (class 32 = sports ball)
and a fine-tuned single-class model without any code path changes.
"""
...
class BallDetector:
    """Tiled YOLO inference for ball detection.
    ...
    def __init__(self, config: Config) -> None:
        ...
        if model_path.exists():
            # User has weights on disk — load directly
        ...
        self._ball_class_id = config.ball_class_id
    ...
    def detect(self, frame: np.ndarray, frame_idx: int = 0) -> list[Detection]:
        ...
        Args:
            ...
            frame_idx: Frame index for populating Detection.frame_idx.

        Returns:
            Deduplicated list of ball detections in frame coordinates.
        """
    ...
    def _compute_tiles(self, h: int, w: int) -> list[tuple[int, int, int, int]]:
        ...
        Returns:
            List of (x1, y1, x2, y2) tile rectangles.
        """
    ...
    def _nms(self, detections: list[Detection]) -> list[Detection]:
        ...
        Args:
            detections: Raw detections from all tiles, frame coordinates.
