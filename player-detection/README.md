...
Pretrained `yolo11m.pt` auto-downloads on first use. EasyOCR's English weights download on first OCR call (~64 MB).

Drop your own footage into `data/raw_clips/`. Video files are gitignored.
...
- `--ocr-every-n N` — sample OCR on 1 in N frames per track (default 5)
...
3. **Sparse OCR** — every `--ocr-every-n` frames per track, crop the player box, run a HSV colour classifier on the upper torso ROI, and run EasyOCR on the same ROI for the digit. Skip crops shorter than `min_bib_crop_h` to avoid wasted OCR on far-end players.
...
```json
{
  ...
  "meta": { "fps": 50.0, "frame_count": 9000, "frame_w": 1920, "frame_h": 1080,
            "model": "yolo11m.pt", "ocr_engine": "easyocr" },
  "frames": [
...
| `src/bib_ocr.py` | **Real** | EasyOCR with `allowlist='0123456789'`, range check 1..12 |
...
- **Detection works.** ~6.2 players/frame on the match section vs. the ~10–14 we expect for 7-a-side — model is missing some far-end players. Tightening `confidence_threshold` lower + enabling `--tiling` should help.
...
- **Bib colours don't match the defaults.** This clip uses **pink and green** bibs; the default HSV ranges are tuned for red/blue. Result: 0/77 identities resolved because every colour reading came back "unknown". To run against this clip, override `Config.hsv_red_h_*` and `Config.hsv_blue_h` to match pink (~150-175) and green (~40-80), or accept that this clip is the wrong test bed for the colour assumption.
- **OCR resolved 8/623 samples (1.3%).** Even with perfect colour matching, OCR is the weakest link at 1080p where bib digits are ~20-40 px tall. Options going forward: (a) train a digit classifier on cropped bib regions, (b) increase input resolution, (c) use a tighter bib-region detector before OCR.
...
- **Bib colour ranges are fixed-width HSV bands.** They assume team colours are saturated red/blue. For other palettes (pink/green, yellow/black, etc.) the HSV ranges in `config.py` need tuning per clip.
